...
import OpenTableLinkInput, { type OpenTableRestaurant } from './OpenTableLinkInput';
...
export default function BookingForm({ onSuccess }: Props) {
  ...
  const [selectedOpenTableRestaurant, setSelectedOpenTableRestaurant] = useState<OpenTableRestaurant | null>(null);
  ...
  const handlePlatformChange = (next: Platform) => {
    ...
    setSelectedOpenTableRestaurant(null);
  };

  const hasRestaurant = platform === 'resy' ? !!selectedRestaurant : !!selectedOpenTableRestaurant;

  // Update booking window when a Resy restaurant is selected (Resy search
  // results carry their own known release schedule; OpenTable links don't).
  useEffect(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (platform === 'resy') {
        ...
        await createReservation({
          restaurantId: selectedOpenTableRestaurant!.slug,
          restaurantName: selectedOpenTableRestaurant!.name,
          platform: 'opentable',
          restaurantSlug: selectedOpenTableRestaurant!.slug,
          targetDate: selectedDate.toISOString().split('T')[0],
          timeRange,
          partySize,
          userEmail: 'user@opentable.com',
          bookingWindow: bookingWindow,
        });
      }
  ...
  // Shared accent per platform - Resy matches the app's existing "primary"
  // blue (used throughout the progress bar/buttons); OpenTable is red.
  const accentBg = platform === 'resy' ? 'bg-primary-600' : 'bg-red-700';
  const accentBgHover = platform === 'resy' ? 'hover:bg-primary-700' : 'hover:bg-red-800';
  const accentBorder = platform === 'resy' ? 'border-primary-600' : 'border-red-700';
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-8 shadow-lg">
      ...
      <div className="flex gap-2 mb-6">
        <button
          onClick={() => handlePlatformChange('resy')}
        ...
        <button
          onClick={() => handlePlatformChange('opentable')}
          className={`flex-1 py-2.5 rounded-lg font-semibold border-2 transition-colors ${
            platform === 'opentable'
        ...
        <div className="space-y-6">
          ...
          {platform === 'resy' ? (
            <>
              ...
              {selectedRestaurant && (
                <div className={`bg-white p-4 rounded-lg border-2 ${accentBorder}`}>
            ...
            <>
              <OpenTableLinkInput onSelect={(r) => {
                setSelectedOpenTableRestaurant(r);
                setStep(2);
              }} />
              {selectedOpenTableRestaurant && (
                <div className={`bg-white p-4 rounded-lg border-2 ${accentBorder}`}>
                  <div className="font-medium text-gray-900">{selectedOpenTableRestaurant.name}</div>
                  <div className="text-sm text-gray-500 truncate">{selectedOpenTableRestaurant.slug}</div>
                </div>
              )}
            </>
          )}
      ...
      {step === 2 && (
        <div className="space-y-6">
          ...
          {selectedDate && (
            <button
              onClick={() => setStep(3)}
              className={`w-full py-3 text-white rounded-lg ${accentBg} ${accentBgHover}`}
            >
              Continue →
            </button>
          )}
      ...
      {step === 3 && (
        <div className="space-y-6">
          ...
          <TimeSelector value={timeRange} onChange={setTimeRange} accent={platform === 'resy' ? 'primary' : 'red'} />
          ...
          {platform === 'resy' ? (
            ...
            <div className="bg-red-50 border-2 border-red-300 rounded-lg p-4 text-sm space-y-1">
              <p className="font-bold text-red-900">🍽️ OpenTable Runner</p>
              <p className="text-red-800 text-xs">
                No auth token needed — OpenTable bookings run through your own signed-in browser
                tab via the Tampermonkey runner. Just make sure it's installed and an{' '}
                <code>opentable.com</code> tab is open when this reservation's booking window opens.
              </p>
            </div>
          )}
          ...
          <button
            ...
            disabled={loading || !timeRange.start || !timeRange.end || (platform === 'resy' && !authToken.trim())}
            className={`w-full py-3 text-white rounded-lg disabled:opacity-50 font-semibold ${accentBg} ${accentBgHover}`}
          >
